        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < certificateDelete.Length; i++)
                {
                    ...
                    {
                        if ((minHeap.GetMin().Key != certificateDelete[i + 1]) && (minHeap.GetMin().Position != 1)) throw new Exception("The min-heap has a wrong structure");
                    }
                    Console.WriteLine(" :: SUCCESS: min-heap's state " + minHeap.ToString());
                }
                result = result + "C";
            ...
            try
            {
                ...
                if (!((node.Position == 1) && (minHeap.GetMin().Key == node.Key))) throw new Exception("The min-heap has a wrong structure");
                Console.WriteLine(" :: SUCCESS: min-heap's state " + minHeap.ToString());
                result = result + "G";
            }
            catch (Exception exception)
